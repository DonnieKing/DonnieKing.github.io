...
window.onload = function () {
    ...
    var countDownWidth = countDown.offsetWidth;//获取倒计时图片宽度
    var timer;
...
//灰太狼随机出现的位置,使用数组来存储对应关系的数据 -- 出现的位置
    ...
    startBtn.onclick = function () {
        ...
        createWolfTimer = setInterval(function () {
            ...
            var resultBol = true; //代表是否随机，是否和上一次狼随机出现的位置一样
            var r = random(0,arrPosi.length - 1);//随机狼位置所在下标
            while(resultBol)
            {
                if (r == preIndex)
                {
            ...
            wolf.onclick = function (){
                ...
                wolf.clickBol = false; //每次点击之后，修改状态，表示已点击
                //清除上升和下降的定时器
                clearInterval(wolf.upTimer);
                clearInterval(wolf.downTimer);
                //重置Index为5，目的是为了处理击打动画
                wolf.index = 5;
                //处理点击狼之后，执行的动画
